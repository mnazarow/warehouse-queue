#!/usr/bin/env bash
# ============================================================================
#  warehouse-queue — установка Rust-варианта на ЧИСТЫЙ сервер (Ubuntu/Debian),
#  без Docker. Ставит всё с нуля: системные пакеты, Rust (rustup), PostgreSQL
#  (создаёт БД и роль), Redis, собирает release-бинарник, поднимает
#  systemd-сервис и (опц.) nginx.
#
...
# ---------------------------------------------------------------------------
log "1/8  Базовые пакеты и зависимости сборки"
apt-get update -y
# build-essential + pkg-config нужны для rusqlite (bundled SQLite собирается из исходников);
# TLS в ureq — на rustls, OpenSSL не требуется.
apt-get install -y git curl ca-certificates ufw build-essential pkg-config
...
# ---------------------------------------------------------------------------
log "3/8  Rust toolchain (rustup для $RUN_USER)"
...
# ---------------------------------------------------------------------------
log "6/8  Код и сборка (release; первая сборка может занять несколько минут)"
